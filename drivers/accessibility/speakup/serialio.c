...
static const struct old_serial_port rs_table[] = {
	SERIAL_PORT_DFNS
};static const struct old_serial_port *serstate;static int timeouts;static int spk_serial_out(struct spk_synth *in_synth, const char ch);static void spk_serial_send_xchar(struct spk_synth *in_synth, char ch);static void spk_serial_tiocmset(struct spk_synth *in_synth, unsigned int set, unsigned int clear);
...
const struct old_serial_port *spk_serial_init(int index) {
	...
	int err; if (index >= ARRAY_SIZE(rs_table)) {
	...
	outb(cval | UART_LCR_DLAB, ser->port + UART_LCR);outb(quot & 0xff, ser->port + UART_DLL);outb(quot >> 8, ser->port + UART_DLM);outb(cval, ser->port + UART_LCR);		/* reset DLAB */
	...
	mdelay(1);speakup_info.port_tts = ser->port;serstate = ser;
